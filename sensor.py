...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    for sensor_data in coordinator.get_sensors_data():
        ...
        # Add specific Installation sensors to the DHW zone (Zone 3)
        if sensor_data.get("zone_id") == 3:
            sensors.append(
    ...
    if installation_devices_data:
        ...
        # Added heat exchanger water outlet temperature
        sensors.append(
            CSNetHomeInstallationSensor(
                coordinator,
                global_device_data,
        ...
        # System Configuration Diagnostic sensors
        sensors.append(
            CSNetHomeInstallationSensor(
                coordinator,
                global_device_data,
...
class CSNetHomeCalculatedSensor(CSNetHomeInstallationSensor):
    ...
    def state(self):
        ...
        # 1. Instant Consumption (Watts) - NEW COMPLEX MODEL WITH GUARDRAILS
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        # Daily COPs
            ...
            if should_accumulate:
                # Accumulate values ONLY when in the correct mode AND accumulating positive energy
                # This prevents accumulation of residual heat (energy_out) when electric power (energy_in) is zero.
...
class CSNetHomeCompressorSensor(CoordinatorEntity, Entity):
    ...
    def state(self):
        ...
        if self._key == "ou_evo_1":
            return heating_status.get("ouEvo1")
