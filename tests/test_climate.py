...
def test_temperature_limit_normalization_air_circuit(hass):
    """Values returned in tenths of °C should be normalized to Celsius."""
    entity = build_entity(hass, mode=1)

    hass.data[DOMAIN][entity.entry.entry_id]["api"] = SimpleNamespace(
        get_temperature_limits=lambda zone_id, mode, data: (190, 350),
        async_set_hvac_mode=AsyncMock(return_value=True),
        async_set_temperature=AsyncMock(return_value=True),
        set_preset_modes=AsyncMock(return_value=True),
        async_set_silent_mode=AsyncMock(return_value=True),
    )
    hass.data[DOMAIN][entity.entry.entry_id]["coordinator"] = SimpleNamespace(
        get_installation_devices_data=lambda: {"heatingStatus": {}},
        get_sensors_data=lambda: [entity._sensor_data],
        get_common_data=lambda: {"device_status": {1234: {"name": "Hitachi PAC", "firmware": "1.0.0"}}},
        async_request_refresh=AsyncMock(return_value=None),
    )

    assert entity.min_temp == 19.0
    assert entity.max_temp == 35.0


def test_temperature_limit_invalid_values_fallback(hass):
    """Sentinel/invalid values should fall back to defaults."""
    entity = build_entity(hass, mode=1)

    hass.data[DOMAIN][entity.entry.entry_id]["api"] = SimpleNamespace(
        get_temperature_limits=lambda zone_id, mode, data: (-5, 0),
        async_set_hvac_mode=AsyncMock(return_value=True),
        async_set_temperature=AsyncMock(return_value=True),
        set_preset_modes=AsyncMock(return_value=True),
        async_set_silent_mode=AsyncMock(return_value=True),
...
def test_temperature_limit_water_circuit_defaults(hass):
    """Water circuits should fallback to water defaults when API data invalid."""
    entity = build_entity(hass, zone_id=5, mode=1)
    ...
    assert entity.min_temp == WATER_CIRCUIT_MIN_HEAT
    assert entity.max_temp == WATER_CIRCUIT_MAX_HEAT


def test_temperature_limit_invalid_order_resets_to_defaults(hass):
    """When min >= max the entity should revert to safe defaults."""
    entity = build_entity(hass, mode=1)

    hass.data[DOMAIN][entity.entry.entry_id]["api"] = SimpleNamespace(
        get_temperature_limits=lambda zone_id, mode, data: (300, 200),
        async_set_hvac_mode=AsyncMock(return_value=True),
        async_set_temperature=AsyncMock(return_value=True),
        set_preset_modes=AsyncMock(return_value=True),
        async_set_silent_mode=AsyncMock(return_value=True),
...
@pytest.mark.asyncio
async def test_cached_limits_reset_after_update(hass):
    """The cached limits should refresh after coordinator update."""
    entity = build_entity(hass, mode=1)

    initial_api = SimpleNamespace(
        get_temperature_limits=lambda zone_id, mode, data: (150, 250),
        async_set_hvac_mode=AsyncMock(return_value=True),
        async_set_temperature=AsyncMock(return_value=True),
        set_preset_modes=AsyncMock(return_value=True),
        async_set_silent_mode=AsyncMock(return_value=True),
    ...
    hass.data[DOMAIN][entity.entry.entry_id]["api"] = initial_api
    hass.data[DOMAIN][entity.entry.entry_id]["coordinator"] = coordinator

    # Prime the cache
    assert entity.min_temp == 15.0
    assert entity.max_temp == 25.0

    # Swap in new limits and ensure async_update invalidates cache
    hass.data[DOMAIN][entity.entry.entry_id]["api"] = SimpleNamespace(
        get_temperature_limits=lambda zone_id, mode, data: (180, 320),
        async_set_hvac_mode=AsyncMock(return_value=True),
        async_set_temperature=AsyncMock(return_value=True),
        set_preset_modes=AsyncMock(return_value=True),
        async_set_silent_mode=AsyncMock(return_value=True),
